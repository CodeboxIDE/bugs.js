...
Runner.prototype.onLine = function(line) {
    ...
    var toRemove = unwanted.filter(function(str) {
        return buffer.indexOf(str) === 0;
    });
    if(toRemove.length > 0) {
        // Remove first string hit of the unwanted
        buffer = buffer.slice(toRemove[0].length);
    }
...
Runner.prototype.isFinished = function() {
    return (
        this.stream._writableSate.ended ||
        this.stream._writableSate.ending
    );
};
